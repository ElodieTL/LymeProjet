...
# Fonction permettant de télécharger des données (fichier ZIP) provenant d'un serveur ou d'un site web.
# url: String représentant l'adresse URL complète de la donnée à télécharger (.ZIP).
# outPath: String représentant le chemin du fichier Zip.
# pathDir: String représentant le chemin des fichiers dé-zippés
def downloadDataZIP(url, outPath, pathDir):
    fileName = os.path.basename(url)
    ...
    z.extractall(path=pathDir)
   filenames = [y for y in sorted(z.namelist()) for ending in ['dbf', 'prj', 'shp', 'shx'] if
                 y.endswith(ending)]
    print(filenames)"""
...
def main():
    ...
    # Télécharger les données de Zone humide
    vectorielles = [] # Créer une liste vide qui contiendra les données vectorielles téléchargées.

    # Répertoire où les données seront enregistrées.
    ZhumideDir = r"Z:\GALAL35\Projet_lyme\Données\Zone Humide"
    ...
    if not os.path.exists(ZhumideDir):
        os.makedirs(ZhumideDir)
    ...
    for url in urlList_ZH:
        # Spécifier les liens vers les fichiers de sortie.
        outPath = os.path.join(ZhumideDir, os.path.basename(url))

        # Si le fichier vectoriel d'origine n'existe pas.
        if not os.path.exists(outPath):
            downloadDataZIP(url,outPath,ZhumideDir)
